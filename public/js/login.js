/**
 * Created by aminelahrim on 02/03/2018.
 */
var emails = document.querySelectorAll('.email'), password = document.querySelectorAll('.password'),
...
//pour isoler le code
var TextAnimation = function (textfiled) {//pour isoler le code
    function getCoord(e, element) {
        var carPos = element.selectionEnd,
            div = document.createElement('div'),
            span = document.createElement('span'),
            copyStyle = getComputedStyle(element),
        ...
        div.textContent = element.value.substr(0, carPos);
        span.textContent = element.value.substr(carPos) || '.';
        div.appendChild(span);

        emailCoords = getPosition(element);							//console.log("emailCoords.x: " + emailCoords.x + ", emailCoords.y: " + emailCoords.y);
    ...
    textfiled.addEventListener('focus', onEmailFocus);
    textfiled.addEventListener('blur', onEmailBlur);
    textfiled.addEventListener('input', onEmailInput);


};

var PassAnimaiton = function (passfiled) {
    ...
    passfiled.addEventListener('focus', onPasswordFocus);
    passfiled.addEventListener('blur', onPasswordBlur);
...
for (var i = 0; i <= emails.length - 1; i++) {
    TextAnimation(emails[i]);
}
for (var i = 0; i <= password.length - 1; i++) {
    PassAnimaiton(password[i]);
}